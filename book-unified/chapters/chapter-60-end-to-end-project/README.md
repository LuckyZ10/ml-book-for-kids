...
class RecommendationRequest(BaseModel):
    ...
    user_id: str
    n_recommendations: int = 10
    algorithm: str = "ensemble"  # user_cf, item_cf, matrix_factorization, ensemble
...
def get_recommendations(request: RecommendationRequest):
    ...
    - **user_id**: 用户ID
    - **n_recommendations**: 推荐数量（默认10）
    - **algorithm**: 推荐算法（user_cf/item_cf/matrix_factorization/ensemble）
    """
    try:
        if request.algorithm == "user_cf":
            recs = cf_model.recommend_user_based(
                request.user_id, 
                n_recommendations=request.n_recommendations
            )
        elif request.algorithm == "item_cf":
            recs = cf_model.recommend_item_based(
                request.user_id,
                n_recommendations=request.n_recommendations
            )
        elif request.algorithm == "matrix_factorization":
            recs = cf_model.recommend_matrix_factorization(
                request.user_id,
                n_recommendations=request.n_recommendations
            )
        else:  # ensemble
            ensemble = RecommendationEnsemble(cf_model)
...
services:
  api:
    ...
    ports:
      - "8000:8000"
    environment:
      - MODEL_PATH=/app/models/cf_model.pkl
    volumes:
      - ./models:/app/models
...
恭喜！你已经完成了60章的学习旅程：
- 从Python基础到深度学习
- 从传统ML到Transformer、LLM
- 从理论到端到端项目

**费曼法一句话总结：**
> 机器学习不是魔法，而是用数据教计算机找规律的艺术。从第一章的"什么是学习"到这一章的完整系统，你已经掌握了这门艺术的精髓。
...
1. **完善推荐系统**：实现冷启动用户的推荐策略
2. **添加评估**：计算推荐系统的Precision@K、Recall@K
3. **部署实践**：将服务部署到云平台（AWS/GCP/阿里云）

### 思考题

1. 如何处理推荐系统的"回音室效应"？
2. 在隐私保护日益重要的今天，联邦学习如何应用于推荐系统？
